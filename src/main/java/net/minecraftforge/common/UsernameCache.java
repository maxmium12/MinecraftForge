...
public final class UsernameCache {
    ...
    {
        final String jsonToWrite = gson.toJson(map);
        ThreadedFileIOBase.getThreadedIOInstance().queueIO(()->{
            try (final BufferedWriter writer = Files.newBufferedWriter(saveFile, Charsets.UTF_8)) {
                writer.write(jsonToWrite);
            }
            catch (IOException e)
            {
                LOGGER.error(USRCACHE, "Error occurred writing username cache file {}", saveFile.toString());
            }
            return false;
        });
    }
